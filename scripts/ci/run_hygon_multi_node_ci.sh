...
install_dtk() {
    local dtk_url="$1"
    local tarball="/opt/$(basename "$dtk_url")"
    local extracted

    if [[ ! -f /opt/dtk/env.sh ]]; then
        curl --fail --location --retry 3 "$dtk_url" --output "$tarball"
...
required=(
    ROCSHMEM_CI_IMAGE ROCSHMEM_CI_DTK_URL ROCSHMEM_CI_SUITE
...
PREPARE_COMMON=(
    "$ROCSHMEM_CI_DTK_URL" "$SSH_PORT" "$PRIMARY_HOST" "$SECONDARY_HOST"
...
ssh "${SSH_OPTIONS[@]}" "$REMOTE" \
    "timeout '$SETUP_TIMEOUT' docker exec \
       ...
       /work/source.tar '$ROCSHMEM_CI_DTK_URL' '$SECONDARY_NIC' '$SECONDARY_NIC_INDEX' \
