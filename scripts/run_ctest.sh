...
DRIVER_SCRIPT="$ROCSHMEM_INSTALL_PREFIX/share/rocshmem/rocshmem_functional_driver.sh"
TESTS_BIN="$ROCSHMEM_INSTALL_PREFIX/share/rocshmem/rocshmem_functional_tests"
...
# Validate label early to prevent path traversal in log directory names
case "$LABEL" in
...
mkdir -p "$LOG_DIR"
# Resolve to absolute path so child processes (driver.sh) can locate the log
# directory regardless of their working directory.
...
# MPI host spec (required for GDA/RO multi-node tests)
...
# Run one backend: run_backend <backend> <mpi_params>
# test_set is selected per backend: ipc->all, gda->gda, ro->all-ro
# hosts_env is set only for multi-node backends (gda/ro); ipc runs single-node.
run_backend() {
    local backend=$1 mpi_params=$2
    local test_set hosts_env=""
    case "$backend" in
        ipc) test_set="ctest-ipc" ;;
        gda) test_set="ctest-gda";   hosts_env="$HOST_SPEC" ;;
        ro)  test_set="ctest-ro"; hosts_env="$HOST_SPEC" ;;
        *)   echo "Error: Unknown backend '$backend'" >&2; return 1 ;;
    esac
    local backend_log_dir="$LOG_DIR/$backend"
    mkdir -p "$backend_log_dir"
    local log="$backend_log_dir/${backend}_${TIMESTAMP}.log"

    echo ""
    echo "====== [${backend^^}] Running Tests (set: $test_set) ======"
    echo "[COMMAND] ROCSHMEM_BACKEND=$backend" \
         "ROCSHMEM_TEST_MPI_PARAMS=\"$mpi_params\"" \
         "ROCSHMEM_TEST_HOSTS=\"$hosts_env\"" \
         "$DRIVER_SCRIPT" \
         "$TESTS_BIN" \
         "$test_set \"$backend_log_dir\""

    local result=0
    ROCSHMEM_BACKEND="$backend" \
    ROCSHMEM_TEST_MPI_PARAMS="$mpi_params" \
    ROCSHMEM_TEST_HOSTS="$hosts_env" \
        "$DRIVER_SCRIPT" "$TESTS_BIN" "$test_set" "$backend_log_dir" 2>&1 \
        | tee "$log" || result=$?

    if [[ $result -eq 0 ]]; then
        echo "✅ ${backend^^} PASSED"
    else
        echo "❌ ${backend^^} FAILED (exit code: $result)"
    fi
    return $result
}
...
for be in $BACKENDS; do
    # run test
    run_backend "$be" "${MPI_PARAMS[$be]}" || RESULTS[$be]=$?
